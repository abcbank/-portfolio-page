...
<DefaultSlide>
    <div class="context" style="color:{color}; font-size:{calcMainFontSize()}rem;">
        <p>IR 동작 검사(Inspector Only)</p>
        <div style="font-size:{calcSubFontSize()}rem; text-align:left; margin-top:20px;">
            <p>
                1. PCB 기판에 전원을 켬<br/>
                2. IR LED를 지정된 주파수로 On/Off<br/>
                3. 기판의 전류값 측정<br/>
                4. 지정된 핀의 전압 주파수 및 최대 / 평균 Pick to Pick 측정<br/>
                &nbsp; &nbsp; &nbsp; &nbsp;- 최대 Pick to Pick는 IR을 켜자마자 바로 발생<br/>
                5. 각 값이 허용치 이내일 경우, 검사 통과<br/>
            </p>
        </div>
    </div>
</DefaultSlide>
